...
#         """
#         # Project input to query, key, value using linear layers
#         # Each projection maintains the dimension D
#         q = self.q_attn(x)  # B x S x D
#         k = self.k_attn(x)  # B x S x D
#         v = self.v_attn(x)  # B x S x D
        
#         # Reshape to split into multiple heads
#         # We split D into H heads, each with dimension HD = D/H
#         q = rearrange(q, 'b s (h hd) -> b h s hd', h=self.n_head)  # B x H x S x HD
#         k = rearrange(k, 'b s (h hd) -> b h s hd', h=self.n_head)  # B x H x S x HD
#         v = rearrange(v, 'b s (h hd) -> b h s hd', h=self.n_head)  # B x H x S x HD
        
#         # Transpose k for efficient matrix multiplication later
#         kT = rearrange(k, 'b h s hd -> b h hd s')  # B x H x HD x S
...
#         # Compute attention scores by multiplying queries with keys
#         # q @ kT gives us the dot product between each query and key
#         qkT = torch.matmul(q, kT)  # B x H x S x S
        
#         # Scale the attention scores to prevent saturation
...
#         s = q.shape[2]  # sequence length
#         causal_mask = torch.tril(torch.ones(s, s, device=q.device)).bool()  # Lower triangular matrix

#         # Handle attention mask for padding tokens
#         if attention_mask is None:
#             mask = causal_mask
#         else:
#             # Combine causal mask with padding mask
#             # attention_mask is B x S, we need to broadcast it properly
...
#             # Add head dimension and combine with causal mask
...
#         # This computes weighted sum of values based on attention scores
#         attn = torch.matmul(attn_weights, v)  # B x H x S x HD
        
#         # Reshape back to B x S x D by concatenating heads
...
#         # First expand to 4x dimension, then project back
#         y = F.gelu(self.linear_in(x))  # B x S x 4D
#         z = self.dropout(self.linear_out(y))  # B x S x D
#         return z
...
#         # Apply layer norm to input, then attention, then add residual
#         attn_out = self.mha(self.ln_1(x), attention_mask)
#         x = x + attn_out  # Residual connection
        
#         # 2. Feed-forward with residual connection
#         # Apply layer norm to current x, then feed-forward, then add residual
...
#         token_embeddings = self.token_embeddings(input_ids)  # B x S x D
        
#         # Calculate position ids
#         if attention_mask is None:
#             # Simple case: positions are [0, 1, 2, ...]
#             position_ids = torch.arange(input_ids.shape[1], device=input_ids.device)
#             position_ids = position_ids.unsqueeze(0).expand_as(input_ids)  # B x S
#         else:
#             # With mask: use cumsum to get positions only for unmasked tokens
#             # cumsum - 1 because positions start at 0
#             position_ids = torch.cumsum(attention_mask.long(), dim=1) - 1
#             # Clamp to avoid out of bounds (masked positions stay at 0)
#             position_ids = position_ids.clamp(min=0)
        
#         # Get positional embeddings
#         positional_embeddings = self.position_embeddings(position_ids)  # B x S x D
        
#         # Combine token and positional embeddings with dropout
...
#         # Use weight sharing: transpose of token embedding matrix
#         logits = F.linear(x, self.token_embeddings.weight)  # B x S x V
#         return logits
...
#         x = self.embed(input_ids, attention_mask)  # B x S x D
        
#         # 2. Pass through all decoder blocks
#         for block in self.blocks:
#             x = block(x, attention_mask)  # B x S x D
        
#         # 3. Final layer normalization
#         x = self.ln(x)  # B x S x D
        
#         # 4. Project to vocabulary logits
#         logits = self.token_logits(x)  # B x S x V
