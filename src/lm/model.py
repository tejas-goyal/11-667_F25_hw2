...
class DecoderLM(nn.Module):
    ...
    def embed(
        ...
        if attention_mask is None:
            position_ids = torch.arange(input_ids.shape[1], device=input_ids.device).unsqueeze(0).expand_as(input_ids)  # B x S
        else:
            position_ids = (attention_mask.cumsum(dim=1) - 1).clamp(min=0).long()  # B x S
    ...
    def forward(
        ...
        #pass x and attention_mask to each decoder block in self.blocks
        for block in self.blocks:
            x = block(x, attention_mask)
        x = self.ln(x) # B x S x D final layer norm
